...
## 31. Virtual Care
- [x] Missing the Top 15 encounters breakdown chart — turned out NOT to be a D3 chart, but a 16-row HTML table (`table.virtual-diagram`) showing each visit type's encounter device, capability flags (text/image/audio/video/vitals/labs/imaging/other), and % of visits. Added a generic `customComponent` block type to the portableText schema with a `name` string field; the renderer dispatches by name to a hard-coded React component. Created `VirtualCareTop15Table.tsx` with all 16 rows extracted from goinvo.com (routine checkup 24.5% → "all other reasons" 50.2%) and inline SVG icons for smartphone/clinic/required-checkmark/optional-checkmark. Inserted the customComponent block into Sanity right after the "The Top 15 Encounters Breakdown" h3.
- [x] Missing time to diagnosis chart — also NOT a D3 chart, just a small 5-row HTML table from Gatsby (Waiting for appointment: 24 days, Waiting for clinician: 41 minutes, Consultation: 18.21 minutes, Total: 24 days and 59.21 minutes). Added `VirtualCareTimeToDiagnosis` component, dispatched via the same `customComponent` block type, inserted into Sanity right after the "On average, it takes 24 days..." paragraph.
