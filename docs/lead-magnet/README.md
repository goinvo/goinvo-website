...
## Build plan (after content approval)

1. **First-party email capture** — `/api/newsletter/subscribe` route calling the
   EmailOctopus API server-side (current embeds are third-party scripts from
   eocampaign1.com: blockable, uninstrumented — zero signup events in GA4 ever).
   Needs `EMAILOCTOPUS_API_KEY` + list ID from the EO dashboard (Shirley has access).
   Tag every signup with source (`premortem-kit`, `doh-page`, …). Fire the
   first-party beacon + GA4 MP event `newsletter_signup`; mark it a key event.
2. **Page build** — article page in the site design system; scorecard as a designed
   print sheet (PDF + print CSS); gated kit download flow (email → EO list w/ tag →
   kit PDF). Public-facing → needs Shirley's review before merge (push policy).
