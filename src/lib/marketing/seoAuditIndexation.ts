...
function unavailableFinding(url: string, reason: string): SeoFinding {
  return makeFinding(
    ...
    'notice',
    'status-unavailable',
    'Index status unavailable (Google Search Console’s URL Inspection tool could not be reached).',
    'Live index status (whether the page is in Google’s search index) comes from Google Search Console, Google’s free site-owner dashboard. It is currently unreachable, so this audit cannot confirm whether Google has this page indexed or which canonical (primary URL) it chose. This is a data-availability gap, not a problem with the page itself.',
    `Confirm the marketing Google service account is configured and has Google Search Console access to this property, then re-run the audit. Diagnostic detail: ${reason}.`,
  ...
  if (verdict !== 'PASS' || !indexed) {
    ...
    findings.push(
      makeFinding(
        ...
        'error',
        'not-indexed',
        `Google has not added this page to its search index (the list of pages it can show in results). Google Search Console reports: "${stateLabel}".`,
        'A page that is not in Google\'s index cannot appear in search results at all — it receives zero organic (unpaid search) traffic no matter how good the content is. This is the single most consequential SEO state.',
        `Open this URL in Google Search Console\'s URL Inspection tool and read the coverage reason ("${stateLabel}"). Common causes: the page is too new (use the tool to request indexing), Google judged it low-value or a duplicate (add more depth or merge it with a similar page), it is blocked from indexing by a noindex tag or the robots file (see the other findings), or it returns a soft 404 (a page that looks empty / "not found" but still reports success). Fix the named cause, then request indexing.`,
        detectedAt,
        false,
      ),
    )
  ...
  if (indexingState === 'BLOCKED_BY_META_TAG' || indexingState === 'BLOCKED_BY_HTTP_HEADER') {
    const mechanism =
      indexingState === 'BLOCKED_BY_META_TAG'
        ? 'a "noindex" tag in the page’s HTML (a robots meta tag set to noindex)'
        : 'a "noindex" instruction in the page’s server response headers (the X-Robots-Tag header)'
    findings.push(
      makeFinding(
        url,
        'warning',
        'noindex',
        `This page is telling Google to keep it out of search results via ${mechanism}.`,
        'A "noindex" instruction deliberately keeps the page out of search results. That is correct for thank-you or admin pages but a silent traffic killer if it is on a page you actually want found.',
        `If this page SHOULD appear in search, remove the "noindex" instruction ${indexingState === 'BLOCKED_BY_META_TAG' ? 'from the page’s HTML <head> section' : 'from the server’s response headers'} and ask Google to re-index it. If it is intentionally hidden, no action needed.`,
        detectedAt,
        false,
      ),
    )
  ...
  if (robotsTxtState === 'DISALLOWED' || indexingState === 'BLOCKED_BY_ROBOTS_TXT') {
    findings.push(
      makeFinding(
        ...
        'warning',
        'robots-blocked',
        `The robots file (robots.txt — the site-wide file that tells crawlers which pages they may visit) is blocking Google from crawling this page.`,
        'A "Disallow" rule in the robots file stops Google from fetching the page, so it cannot read the content or reliably index it. Unlike a "noindex" instruction, the URL can still appear as a bare, description-less result.',
        `Check the robots file (robots.txt) for a "Disallow" rule that matches this URL and remove or narrow it if this page should be crawlable. If you only want it out of search results, use a "noindex" instruction instead of a robots-file block (a blocked page can’t even be told it has a "noindex" instruction).`,
        detectedAt,
        false,
      ),
    )
  ...
  if (googleCanonical && userCanonical && googleCanonical !== userCanonical) {
    findings.push(
      makeFinding(
        ...
        'warning',
        'canonical-mismatch',
        `Google disagrees with this page about which URL is the primary one. The canonical tag tells search engines which URL is the primary version of a page — Google picked "${googleCanonical}", but this page declares "${userCanonical}".`,
        'When Google ignores your declared canonical (primary URL) and picks a different URL, ranking signals consolidate onto Google\'s pick, not yours — your preferred URL may never be the one shown or ranked.',
        `Decide which URL is truly the primary one. If "${userCanonical}" is correct, strengthen the signal (link to it consistently from your own pages, include it in the sitemap, and use the same canonical tag on any duplicate pages) so Google trusts it over "${googleCanonical}". If Google\'s pick "${googleCanonical}" is actually the better URL, update this page\'s canonical tag to match it.`,
        detectedAt,
        true,
      ),
    )
  ...
  if (pageFetchState && pageFetchState !== 'SUCCESSFUL') {
    findings.push(
      makeFinding(
        ...
        'error',
        'fetch-failed',
        `Google could not successfully load this page (Google reported the fetch state: ${pageFetchState}).`,
        'If Googlebot (Google’s web crawler) cannot load the page, Google cannot index its content. A soft 404 (a page that looks empty / "not found" but still reports success), a server error, or a redirect at load time blocks the page from ranking regardless of its quality.',
        `Reproduce what Googlebot saw: ${pageFetchState === 'SOFT_404' ? 'the page reported success (HTTP 200) but looks empty or like a "not found" page — add real content, or return a proper 404 / redirect' : pageFetchState === 'NOT_FOUND' ? 'the page returns a 404 "not found" error — restore the page or redirect the URL' : `the load state was ${pageFetchState} — check the server response, any redirects, and any crawler block`}. Then use Google Search Console’s URL Inspection "Test Live URL" button to confirm a clean load.`,
        detectedAt,
        false,
      ),
    )
  ...
  if (findings.length === 0) {
    findings.push(
      makeFinding(
        ...
        'notice',
        'indexed-ok',
        `Google has this page in its search index${coverageState ? ` ("${coverageState}")` : ''}.`,
        'The page is in Google\'s index with no "noindex" instruction, robots-file block, canonical (primary-URL) conflict, or load problem — the healthy state for a page you want found.',
