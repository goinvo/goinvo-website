...
/* Inline brand SVGs so the ported `.social-buttons svg` rule (28x28 #474751
   chip, #F2F2F2 fill, 6px padding, hover #83838E) mounts — matching the
   original GoInvo legacy feature's share icons. The chip styling lives on the
   <svg> element; the glyph path inherits `fill` from the CSS. */
...
const SHARE_URL = 'http://www.goinvo.com/vision/bathroom-to-healthroom/'
  ...
  return (
    <div className="healthroom-legacy pt-[var(--spacing-header-height)]">
      ...
      <article className="design-for-life" id="feature-article">
        {/* Title section — tall video header with a left-aligned dark
            `.header-contents` block pinned to the top, fronted by a white
            top-fading gradient scrim (mirrors the legacy from-bathroom-to-
            healthroom feature). The background video sits behind everything;
            `.header-contents` re-flows into the padding the .bathroom-header
            rule reserves. */}
        <header className="bathroom-header relative overflow-hidden">
          <BackgroundVideo
            mp4Src={`${VID_BASE}/header.mp4`}
            webmSrc={`${VID_BASE}/header.webm`}
            posterSrc="/videos/dfl/header.jpg"
          />
          <div className="header-contents relative z-[1]">
            ...
            <div className="social-buttons">
              ...
              <a
                ...
                href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(
                  SHARE_URL,
                )}&hashtags=healthcare,design&via=goinvo`}
                target="_blank"
                rel="noopener noreferrer"
              ...
              <a
                ...
                href={`http://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(
                  SHARE_URL,
                )}&title=From%20Bathroom%20to%20Healthroom`}
                target="_blank"
                rel="noopener noreferrer"
